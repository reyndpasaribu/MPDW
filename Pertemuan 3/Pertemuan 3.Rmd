...
cons_lm1 <- dynlm(close ~ open+L(open),data = train.ts)
#sama dengan model ardl p=1 q=0
cons_lm2 <- dynlm(close ~ open+L(close),data = train.ts)
#sama dengan ardl p=1 q=1
cons_lm3 <- dynlm(close ~ open+L(open)+L(close),data = train.ts)
#sama dengan dlm p=2
cons_lm4 <- dynlm(close ~ open+L(open)+L(open,2),data = train.ts)
#sama dengan ardl
```
...
dwtest(cons_lm2)  #Ada autokorelasi
...
#Kenormalan
```{r}
shapiro.test(residuals(cons_lm1))  #Tidak Normal
shapiro.test(residuals(cons_lm2))  #Normal
shapiro.test(residuals(cons_lm3))  #Normal
shapiro.test(residuals(cons_lm4))  #Tidak Normal 
```
#Perbandingan Model
```{r}
